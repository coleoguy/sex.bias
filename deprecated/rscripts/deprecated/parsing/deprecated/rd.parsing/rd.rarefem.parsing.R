...
load("../sex.bias/rscripts/results/rd.rare.females.RData")
...
# res.rare.fem$females500$males0.2$rd0$h0$s0.1
...
for(i in 1:2){
  ...
  for(j in 1:3){
    ...
    for(k in 1:25){
      if(names(res.rare.fem[[i]][[j]])[k] == "rd0.48"){
...
write.csv(results.fit.h0, file ="rd0.48.h0.rfem.sexchrom.csv")
write.csv(results.fit.h0.5, file ="rd0.48.h0.5.rfem.sexchrom.csv")
write.csv(results.fit.h1, file ="rd0.48.h1.rfem.sexchrom.csv")
