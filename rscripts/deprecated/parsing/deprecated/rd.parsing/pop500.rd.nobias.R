...
colnames(results.fit.h0.5) <- c("500")
...
write.csv(results.fit.h0, file ="pop500.h0.nobias.XYmodel.csv")
write.csv(results.fit.h0.5, file ="pop500.h0.5.nobias.XYmodel.csv")
write.csv(results.fit.h1, file ="pop500.h1.nobias.XYmodel.csv")
...
no.bias.h0 <- read.csv("../sex.bias/rscripts/parsing/rd.parsing/nobias/pop500.h0.nobias.XYmodel.csv", 
                       row.names = 1, as.is = T, header = T, check.names = F)
no.bias.h05 <- read.csv("../sex.bias/rscripts/parsing/rd.parsing/nobias/pop500.h0.5.nobias.XYmodel.csv", 
                        row.names = 1, as.is = T, header = T, check.names = F)
no.bias.h1 <- read.csv("../sex.bias/rscripts/parsing/rd.parsing/nobias/pop500.h1.nobias.XYmodel.csv", 
                       row.names = 1, as.is = T, header = T, check.names = F)
...
merged.col.names <- c("rd", "500.h0", "500.h0.5", "500.h1")
...
# save them
write.csv(melted.dat, file = "pop500.no.bias.melted.data.csv")
...
test <- read.csv("../sex.bias/figures/Fitness.plots/XY.model/rd.plots/pop500.no.bias.melted.data.csv", 
                 row.names = 1, as.is = T, header = T, check.names = F)
...
Population size = 500. s = 0.5") + xlab("recombination distance (rd)") + 
