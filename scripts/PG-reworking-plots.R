...
str(dat)
dat <- dat[,2:10] #remove X.1 if rows weren't saved properly
  ...
  for(j in 1:7){#OSRs
    for(k in 1:4){#hs
      for(l in 1:3){#ss
        ...
        #Temperary file to store data in for calculations for mean.results
        #Can we aggregate data?
        temp.cur<-aut[aut$females ==pop[i],] #all columns with female with common number
        temp.cur<-temp.cur[temp.cur$OSR ==osrs[j],] #keep those with set OSR level
        temp.cur<-temp.cur[temp.cur$h == hs[k],] #keep those with set dominance level
        ...
        mean.results.rare.male.aut[row.num, 5]<-mean(temp.cur$gens) #Average number of generations over the simulations for a specific time point, col 5
        ...
        mean.results.rare.male.aut[row.num, 8]<-sum(temp.cur$A == 0)/length(temp.cur$A) #prob deleterious? 
        
        #going manually row by row
        row.num<-row.num + 1
      }
    }
  }
