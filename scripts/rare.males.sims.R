...
# allele 1 is fit for females
# allele 2 is fit for males
...
colnames(results) <- c("X","Y","A","females","OSR", "rd","h","s", "gens")

# this runs the rare female conditions for our sim project
      ...
      for(k in 1:length(h)){
        ...
        for(m in 1:length(s)){
          ...
          x <- foreach(n=1:iter, .combine = "rbind") %dopar%{
            # these next two lines handle figuring out how many females
            # we have based on male count and current bias level
            males <- ceiling(females[i]*bias[ii])
            ...
            # frequencies of each genotype in females and males
          ...
          results[result.counter:(result.counter+iter-1), 4] <- females[i]
