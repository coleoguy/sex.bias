...
# create matrix for results when there is bias with genetic architectures 
# h = 0, 0.5, 1, and  sex- specific dominance. Looking at the case when 
# males are the common sex.
...
# Evaluation of exually antagonistic loci on sex  chromosomes when there is 
# bias.
# structure of results is as follows: 
# res.rare.fem$males50$females0.8$rd0.1$h0$s0.5 
# (loop will start iterating in this order in the case of sex chromosomes.)
# cycle through population sizes (50, 100, 500, 1000)  
for(i in 1:4){
  ...
  for(j in 1:6){
     ...
     for(k in 1:4){
      if(names(res.rare.fem[[i]][[j]])[k] == "rd0.1"){
        # cycle through genetic architectures. The selection coefficient 
        # is hard coded. If you want to choose a different
        # coefficient you need to change '$s0.5' to any of the values we 
        # have used for the simulations(0.1, 0.2, 0.5, 0.9).
        for(m in 1:4){
          if(m == 1){
            for(n in 1:4){
              fit <- getFitness(data = res.rare.fem[[i]][[j]][[k]][[m]]$s0.5, 
                                loc, h[m], s)
...
# Since the results of these  loops are one table for difference in relative 
# fitness when there is no bias, and four tables for when there is bias 
# (one for each genetic architecture), need to extract the relevant row from 
# the "no bias" table for each genetic architecture, and then row bind the 
# matching "bias" table.
...
# create matrix for results when there is bias with genetic architectures 
# h = 0, 0.5, 1, and  sex-specific dominance. 
...
# Need to run through this loop once more since the getDifference function 
# for the case where females are the common sex, is different.
# Sexually antagonistic loci evaluation on sex chromosomes
# cycle through population sizes (50, 100, 500, 1000)
for(i in 1:4){
  # pull results when rd = 0.1 only
...
# Evaluation of exually antagonistic loci on sex  chromosomes when 
# there is bias
# structure of results is as follows: 
# res.rare.fem$males50$females0.8$rd0.1$h0$s0.5 
# (loop will start iterating in this
...
# Since the results of these  loops are one table for difference in relative 
# fitness when there is no bias, and four tables for when there is bias 
# (one for each genetic architecture), need to extract the relevant row 
# from the "no bias" table for each genetic architecture, and then row bind 
# the matching "bias" table.
